...
let winner = false;
alert(`Welcome to Dungeons and Dragons! Are you ready to fight and kill the ogre that has been killing people around your town? 
    ...
    Let's try it out before starting the game. Press OK.`);
...
alert("So... Where were we? Yes... You are an elf warrior that decided that today's the day for the ogre to stop messing with the people around your village. So you take the golden path to the forest in order to find and slay him. You see him right outside a cave while he's eating God-knows-what. You are aeger to strike him at once and when he doesn't even expect it. Roll to see if you succed to attack him!")
...
while (!winner) {
    if (userHealth > 0 && ogreHealth > 0) { //works!! BUT SHOWS NEGATIVE VALUE FIRST AND THEN EXITS THE LOOP 
